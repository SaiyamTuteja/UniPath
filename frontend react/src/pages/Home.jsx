...
import UniPathlogo from "./assets/images/Gehu.png";
...
// --- MODIFIED: Added FaGraduationCap ---
...
// --- NEW: Import FaSearch from the correct 'fa' package ---
...
function Home() {
  ...
  const serverlink = "https://UniPath.azurewebsites.net";
  const serverlink2 = "https://class-sync-azure.azurewebsites.net";
  ...
  const [room_status_data, set_room_status_data] = useState(
    localStorage.getItem("roomstatus_infocache")
      ? JSON.parse(localStorage.getItem("roomstatus_infocache"))
      : null
  );
  ...
  const fetchGeoJSON = async () => {
    ...
    try {
      ...
      if (
        mapData &&
        mapsetdate &&
        Date.now() - mapsetdate < 24 * 60 * 60 * 1000 * 7
      ) {
        // 7 days
  ...
  const fetch_calculate_antpath = async () => {
    ...
    if (roomData && Array.isArray(roomData.rooms)) {
      for (const floor of roomData.rooms) {
        if (floor.room_data.find((room) => room.roomid == source)) {
          if (floor.floor_label === "Ground floor") sourceFloor = 0;
          else if (floor.floor_label === "1st floor") sourceFloor = 1;
          else if (floor.floor_label === "2nd floor") sourceFloor = 2;
          else if (floor.floor_label === "3rd floor") sourceFloor = 3;
          else if (floor.floor_label === "4th floor") sourceFloor = 4;
          else if (floor.floor_label === "5th floor") sourceFloor = 5;
          else if (floor.floor_label === "Underground floor") sourceFloor = -1;
          break;
        }
      }
    }
    if (sourceFloor === 0 && source != 0) {
      try {
        let floorNum = Math.floor(parseInt(source) / 1000);
        if (floorNum >= 1 && floorNum <= 6) sourceFloor = floorNum - 1;
        if (parseInt(source) < 1000) sourceFloor = -1;
      } catch (e) {
        console.error("Could not parse source floor");
      }
    }

    try {
      const url = `${serverlink}/getCoordinates?src=${source}&des=${destination}&accessible=${isAccessible}`;
      ...
      let message =
        tempTime < 60 ? "~1 min" : `${Math.ceil(tempTime / 60) + 1} min`;
      setGobuttontext(message);
      setActiveFloor(sourceFloor);
    } catch (error) {
      console.error("Error fetching path coordinates:", error);
      toast.error(`Could not find path. ${error.message}`);
      setpathPoints([[], [], [], [], [], [], []]);
      throw error;
    } finally {
      LoaderManager(0);
    ...
    try {
      ...
      if (
        ((today.getHours() >= 18 && today.getHours() <= 23) ||
          (today.getHours() >= 0 && today.getHours() <= 7)) &&
        stored_status
      ) {
        toast.success("Campus is closed. Showing last known data", {
          id: toastid,
        });
        return;
      }
      ...
      if (
        stored_status &&
        roomstatus_setdate &&
        Date.now() - new Date(roomstatus_setdate).getTime() < 7 * msInDay
      ) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(checkHour, 60 * 1000 * 5);
  ...
  const getSpecificRoomCoordinates = (floordata, room_id) => {
    let coordinates = [];
  ...
  }, [source, destination, isAccessible]);
      ...
      const fetchSearch = async () => {
        try {
          const response = await fetch(
            `${serverlink}/search?q=${debouncedSearchQuery}`
          );
          const data = await response.json();
  ...
  // --- NEW: "Go to My Next Class" Function ---
  const handleGoToNextClass = async () => {
    ...
    const toastid = toast.loading("Finding your next class...");
    LoaderManager(1);
    try {
      const response = await fetch(`${serverlink}/api/v1/schedule/next-class`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      if (data.success && data.data.found) {
        const { roomid, roomName, subjectName } = data.data.class;
        setdestination(roomid);
        toast.success(`Navigating to: ${subjectName || roomName}`, {
          id: toastid,
        });
      } else {
        toast.error(data.data.message || "Could not find your next class.", {
          id: toastid,
        });
      }
    } catch (error) {
      console.error("Next Class Error:", error);
      toast.error("Error finding schedule. Please try again.", { id: toastid });
    } finally {
      LoaderManager(0);
    }
  };
  ...
  if (!user) {
    return (
      <div className="flex justify-center w-full h-full position-fixed align-items-center">
        <div className="flex flex-col items-center justify-center">
          <img src={gif} alt="Loading animation" className="h-50 w-50" />
          <p className="mt-2 text-xl text-brand-primary-dark fw-bold animate-pulse">
            Signing in
    ...
    <div>
      ...
      {showScanner && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-[1001]">
          <div className="flex w-3/4 max-w-lg rounded-lg shadow-2xl">
            <button
              onClick={() => setShowScanner(false)}
              className="absolute p-3 text-white transition rounded-full top-2 right-2 hover:bg-black hover:bg-opacity-50"
            >
              ...
              onScan={(detectedCodes) => {
                if (detectedCodes.length > 0) {
                  ...
                  try {
                    ...
                    const scannedSource =
                      scanneddata.searchParams.get("source");
                    const scannedDestination =
                      scanneddata.searchParams.get("destination");
      ...
      <div className="position-fixed bottom-0 fw-bold left-0 text-lg text-brand-primary-dark px-2 fw-bold z-[1]"></div>
      <div className="position-fixed bottom-0 fw-bold right-0 text-lg text-brand-primary-dark p-1 fw-bold z-[1]">
        {signalStrength === 0 ? (
          ...
          <FaCircle
            className={`shadow-2xl drop-shadow-2xl text-md animate-pulse  ${
              signalStrength === 1
                ? "text-[red]"
                : signalStrength === 2
                ? "text-orange"
      ...
      <nav className="flex align-items-center p-2 position-fixed z-[1] w-full top-0 gap-3">
        <div className="flex align-items-center">
          <img className="h-[80px]" src={UniPathlogo} height="70px" />
          <h1 className="mx-2 text-4xl text text-brand-primary-dark">
            <b>UniPath</b>
          </h1>
        </div>
        ...
        <div className="relative flex-grow max-w-lg mx-auto">
          <div className="flex items-center p-2 bg-white rounded-full shadow-lg">
            ...
            <input
              type="text"
              placeholder="Search for offices, labs, cafes..."
              className="flex-grow bg-transparent border-none outline-none"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            {isSearchLoading && (
              <div className="spinner-border spinner-border-sm text-brand-primary-dark me-2"></div>
            )}
          </div>
          {searchResults.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-xl max-h-60 overflow-y-auto allow-browser-menu z-[1002]">
                  ...
                  >
                    <div>
                      ...
                      <small className="text-muted">
                        {result.label} ({result.category}) - Floor{" "}
                        {result.floor}
                      </small>
                    </div>
                    <div className="gap-1 d-flex">
                      <button
                        ...
                        onClick={() => {
                          setsource(result.node);
                          setSearchQuery("");
                          setSearchResults([]);
                          toast.success(`Start set to: ${result.name}`);
                        }}
                      >
                        Start
                      </button>
                      <button
                        ...
                        onClick={() => {
                          setdestination(result.node);
                          setSearchQuery("");
                          setSearchResults([]);
                          toast.success(`Destination set to: ${result.name}`);
                        }}
                      >
                        End
                      </button>
      ...
      >
        <FontAdjuster />
        <TileLayer
          attribution='<p><a href="https://github.com/SaiyamTuteja"><img src="https://flagcdn.com/in.svg" width="15" alt="India"><b> Made by Ayush Saklani</b></a> <br>Co-powered by <a href="https://github.com/SaiyamTuteja/classsync"><b>Classsync</b></a></p>'
        ...
        {
          // antpath
          pathPoints[activeFloor + 1] &&
            pathPoints[activeFloor + 1].length > 0 && (
              <AnimatedPolyline
        ...
        )}
        {
          ...
          room_status_data &&
            floorMap &&
            floorMap.map((floordata, index) => {
              if (floordata.floor === activeFloor.toString()) {
                return floordata.map.features.map((feature) => {
                  if (
                    feature.properties?.room_id &&
                    room_status_data?.find(
                      (room) => room.roomid === feature.properties.room_id
                    )
                  ) {
                    const room_talking_about = room_status_data?.find(
                      (room) => room.roomid === feature.properties.room_id
                    );
                    const tempvar =
                      room_talking_about.schedule[dayslot.toLocaleLowerCase()][
                        hourslot.toLocaleLowerCase()
                      ];
                    return (
                      <div key={feature.properties.room_id}>
                        <Polygon
                          key={feature.properties.room_id}
                          positions={getSpecificRoomCoordinates(
                            floordata.map,
                            feature.properties.room_id
                          )}
                          color={
                            map_color_set[room_talking_about.type]
                              ? map_color_set[room_talking_about.type].color
                              : isActivehour()
                              ? map_color_set["closed"].color
                              : room_talking_about.schedule[
                                  dayslot.toLocaleLowerCase()
                                ][hourslot.toLocaleLowerCase()].section.length >
                                0
                              ? map_color_set["occupied"].color
                              : map_color_set["available"].color
                          }
                          opacity={opacity}
                          weight={weight}
                          fillColor={
                            map_color_set[room_talking_about.type]
                              ? map_color_set[room_talking_about.type].fillColor
                              : isActivehour()
                              ? map_color_set["closed"].fillColor
                              : room_talking_about.schedule[
                                  dayslot.toLocaleLowerCase()
                                ][hourslot.toLocaleLowerCase()].section.length >
                                0
                              ? map_color_set["occupied"].fillColor
                              : map_color_set["available"].fillColor
                          }
                          ...
                          eventHandlers={{
                            contextmenu: (e) => {
                              ...
                              if (room_talking_about.type == "ladieswashroom") {
                                ...
                                room_talking_about.type == "gentswashroom"
                              ) {
                                setdestination(1999);
                              } else {
                              ...
                              toast.success(
                                `Destination set to: ${room_talking_about.name}`
                              );
                            },
                          }}
                        >
                              ...
                              <Info_Card
                                ...
                                section={
                                  !isActivehour()
                                    ? room_talking_about.schedule[
                                        dayslot.toLocaleLowerCase()
                                      ][hourslot.toLocaleLowerCase()].section
                                    : ""
                                }
                                subjectcode={
                                  !isActivehour()
                                ...
                                semester={
                                  !isActivehour()
                                    ? room_talking_about.schedule[
                                        dayslot.toLocaleLowerCase()
                                      ][hourslot.toLocaleLowerCase()].semester
                                    : ""
                                }
                                infotype={
                                  room_talking_about.type == "ladieswashroom" ||
                                  room_talking_about.type == "gentswashroom"
                                    ? "washroom"
                                    : isActivehour()
                                    ? "closed"
                        ...
                        {!user.guest &&
                          ...
                          user.course &&
                          user.semester &&
                          tempvar.section.includes(user.section) &&
                          tempvar.course == user.course &&
                          tempvar.semester == user.semester && (
                            <AnimatedPolyline
                              positions={getSpecificRoomCoordinates(
                                floordata.map,
                                feature.properties.room_id
      ...
      <div className="floorbutton mx-3 flex gap-1 justify-content-center align-items-center z-[1] flex-column position-fixed  left-0 top-[14%]">
        {floors.map((floor, index) => (
          <button
            key={index}
            className={`bg-brand-primary-light text-white dark:bg-brand-primary-dark dark:text-white border-3 border-brand-primary-dark ${
              activeFloor === floor &&
              "dark:bg-brand-primary-light dark:border-brand-primary-dark"
            } dark:border-brand-primary-light
                        circular_button aspect-square ${
                          activeFloor === floor ? "active" : ""
                        }`}
            id={floor}
            onClick={() => setActiveFloor(floor)}
          >
            <b>{floor === -1 ? "B" : floor === 0 ? "G" : floor}</b>
          </button>
        ))}
      </div>
      {/* floorbutton */}
      ...
      <div className={`bottom-bar p-0 ${down ? "bottom-bar-down" : ""}`}>
        <button
          className="countboxi text-brand-primary-dark"
          onClick={handleClick}
        >
          <i
            id="foldup-icon"
            className={`bi ${
              down ? "bi-caret-up-fill" : "bi-caret-down-fill"
            } h3`}
          ></i>
        </button>
        <div className="container my-auto row justify-content-center">
          <div className="pb-1 form-floating col-lg-5 col-md-5 col-sm-12 text">
            <select
              className="form-select"
              id="Start"
              onChange={() => {
                setsource(document.getElementById("Start").value);
              }}
              value={source}
            >
              ...
              {roomData.rooms.map((room, index) => (
                <optgroup label={room.floor_label} key={index}>
                  {room.room_data.map((subroom, subindex) => (
                    <option key={subindex} value={subroom.roomid} style={{}}>
                      {subroom.room_name}
                    </option>
                  ))}
                </optgroup>
            ...
            >
              <BsQrCodeScan className="p-2 mx-4 text-5xl bg-white cursor-pointer position-absolute top-50 end-0 translate-middle-y text-brand-primary-dark hover:bg-brand-primary-dark" />
            </div>
          </div>
          <div className="pb-1 form-floating col-lg-5 col-md-4 col-sm-12 text">
            <select
              className="h-full form-select"
              id="destination"
              onChange={() => {
          ...
          <div className="gap-2 pb-1 col-lg-2 col-md-3 col-sm-12 align-items-center justify-content-center d-flex">
            <button
              type="button"
              className="h-full px-2 btn btn-lg btn-light col-8 text-brand-primary-dark"
              id="go"
              onClick={() => {
                if (source == 0 || destination == 0) {
            ...
            <div className="h-full p-0 form-check form-switch d-flex flex-column align-items-center justify-content-center col-4">
              <input
              ...
              />
              <label
                className="form-check-label text-brand-primary-dark fw-bold"
      ...
      <footer className="fixed top-[90px] right-0 m-1 z-[1] flex flex-col gap-1 align-items-end justify-content-center">
        <div className="px-1 text-3xl text-brand-primary-dark fw-bold">
          {loading == false ? (
            <div className="py-1 text-2xl text-brand-primary-dark">
              {timevar}
            </div>
          ) : (
        ...
        <div>
          <Popover className="relative flex flex-col gap-1 center">
            <PopoverButton>
              {user.guest ? (
                <img
                  src={profilepicture}
                  alt=""
                  size={44}
                  className="w-12 h-12 p-0 rounded-full cursor-pointer"
                />
              ) : (
                <img
            ...
            <PopoverPanel
              anchor="bottom"
              className="flex flex-col bg-brand-primary-light rounded-xl z-[1001]"
            >
        ...
        <a href="https://github.com/SaiyamTuteja" target="_blank">
          <FaGithub
            size={44}
            className="p-2 bg-brand-primary rounded-xl hover:bg-brand-primary-light text-foreground-1"
          />
        </a>
        ...
        {/* --- NEW: "Go to My Next Class" Button --- */}
        {/* Only show if the user is not a guest */}
        {!user.guest && (
          <div title="Go to My Next Class">
            <FaGraduationCap
              size={44}
              className="p-2 text-3xl cursor-pointer bg-brand-primary rounded-xl hover:bg-brand-primary-light text-foreground-1"
              onClick={handleGoToNextClass}
            />
          </div>
        )}

        <div title="Reset Path">
          <FaArrowRotateRight
            size={44}
            className="p-2 text-3xl cursor-pointer bg-brand-primary rounded-xl hover:bg-brand-primary-light text-foreground-1"
            onClick={() => {
